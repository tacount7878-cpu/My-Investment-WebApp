...
def check_login():
    ...
    # 顯示登入表單
    st.markdown("## 🔐 請先登入雲端戰情室")
    
    with st.form("login_form"):
        username = st.text_input("帳號 (Username)")
        password = st.text_input("密碼 (Password)", type="password")
        submit = st.form_submit_button("登入 Login")
        ...
        if submit:
            ...
            try:
                ...
                st.error("⚠️ 尚未在 Secrets 設定 [credentials] 帳密，請檢查設定！")
                return False
...
with st.sidebar:
    st.success(f"👤 已登入: {st.secrets['credentials']['username']}")
    st.divider()
    st.header("🔄 數據同步")
...
def main():
    ...
    nav = st.radio(
        label="", options=NAVS, horizontal=True,
    ...
    if nav == "📊 視覺化分析":
        ...
        if not df_h.empty:
            ...
            with c_p1: 
                if "投資地區" in df_h.columns: st.plotly_chart(px.pie(df_h, values="市值(TWD)", names="投資地區", title="投資地區佔比", hole=0.4), use_container_width=True)
            with c_p2: 
                if "合併鍵(GroupKey)" in df_h.columns: st.plotly_chart(px.pie(df_h, values="市值(TWD)", names="合併鍵(GroupKey)", title="資產類別佔比", hole=0.4), use_container_width=True)
    ...
    # ======================================================
        ...
        with st.form("trade_form", clear_on_submit=True):
            ...
            if symbols:
                ...
                d_sym = st.text_input("股票代號", value="")
            ...
            if st.form_submit_button("送出交易"):
                if (not d_sym) or (symbols and d_sym not in symbols):
                    st.error("找不到該股票代號資料 (請確認 holdings)")
    ...
    # ======================================================
        ...
        if not df_l.empty:
            ...
            show_cols = ["日期", "交易類型", "股票代號", "幣別", "賣出價格", "賣出股數", "成本(原幣)※賣出需填", "價金(原幣)", "應收付(原幣)", "損益(原幣)", "報酬率"]
